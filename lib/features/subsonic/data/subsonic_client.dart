...
class SubsonicClient {
  ...
  Future<List<SubsonicPlaylist>> getPlaylists() async {
    final response = await _getJson('getPlaylists');
    final root = response['playlists'] as Map<String, dynamic>?;
    if (root == null) return [];

    final rawList = root['playlist'];
    final List<dynamic> playlistList;
    if (rawList is List) {
      playlistList = rawList;
  ...
  Future<List<SubsonicSong>> getPlaylist(String playlistId) async {
    final response = await _getJson('getPlaylist', {'id': playlistId});
    final playlistData = response['playlist'] as Map<String, dynamic>?;
    if (playlistData == null) return [];

    final rawEntry = playlistData['entry'];
  ...
  Future<Uint8List?> getCoverArtBytes(
    String coverArtId, {
    int size = 500,
  }) async {
