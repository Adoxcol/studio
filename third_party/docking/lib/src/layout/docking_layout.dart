...
class DockingLayout extends ChangeNotifier {
  ...
  void moveItem(
      ...
      int? dropIndex}) {
    // Reordering within a strip must not replace every parent in the layout:
    // doing so destroys the strip's animation/drag state and rebuilds all panes.
    if (targetArea is DockingTabs &&
        draggedItem.parent == targetArea &&
        dropIndex != null &&
        dropPosition == null) {
      if (draggedItem.layoutId != id || targetArea.layoutId != id) {
        throw ArgumentError('Items must belong to this layout.');
      }
      RangeError.checkValueInInterval(dropIndex, 0, targetArea.childrenCount);
      final oldIndex = targetArea._children.indexOf(draggedItem);
      final newIndex = dropIndex > oldIndex ? dropIndex - 1 : dropIndex;
      if (oldIndex == newIndex) return;
      final selected = targetArea.childAt(targetArea.selectedIndex);
      targetArea._children.removeAt(oldIndex);
      targetArea._children.insert(newIndex, draggedItem);
      targetArea.selectedIndex = targetArea._children.indexOf(selected);
      _updateHierarchy();
      notifyListeners();
      return;
    }
