...
class _TabsAreaLayoutRenderBox extends RenderBox
  ...
  // Animate the actual parent offsets, so hit testing and semantics always
  // agree with the painted tabs. No panel rebuilds/layout work on each tick.
  void _tick() {
  ...
  @override
  void performLayout() {
    final previousOffsets = <RenderBox, Offset>{};
    visitChildren((child) {
    ...
    if (!_animate) {
      ...
      _starts = {
        for (final entry in targets.entries)
          entry.key: previousOffsets[entry.key] ?? entry.value,
      };
      _targets = targets;
      ...
      _targets = targets;
      _starts.removeWhere((child, _) => !targets.containsKey(child));
      if (_animation.isAnimating) _tick();
    }
  }
